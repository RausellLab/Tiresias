...
class RwrM:
    ...
    adjacency_matrices: torch.FloatTensor of size n_layers * n_nodes * n_nodes
        Tensor containing the adjacency matrices of the multilayer graph on which the
        model will be applied. Each adjacency matrix corresponds to a layer of the
        graph.
    ...
    def fit(self, train_labels, train_mask):
        ...
        prev_pred = torch.zeros(
            self.n_nodes * self.n_layers, dtype=torch.float, device=self.device
        )
        ...
        for i in range(self.max_iter):
            ...
            self.pred = (1 - self.r) * torch.matmul(
                self.transition_matrix, self.pred
            ) + self.r * init_pred
...
def build_transition_matrix(adjacency_matrices, delta):
    ...
    norm_I = (
        delta / (n_layers - 1) * torch.eye(n_nodes, dtype=torch.float, device=device)
    )
    ...
    for row in range(n_layers):
        ...
        for col in range(n_layers):
            ...
            if row == col:
                transition_matrix[start_row:end_row, start_col:end_col] = (
                    1 - delta
                ) * adjacency_matrices[row]
            else:
